...
//	gStyle->SetOptStat(kFALSE);
	...
	petAnalysis_Energy->SetAxisRange(0., 15000.0,"X");

	TF1* gauF2 = new TF1("gauF2","gaus",0,10000);
	petAnalysis_Energy->Fit("gauF2","","e",8000,10000);

	petAnalysis_Energy->Draw();


	TCanvas* c2	= new TCanvas("c2", "First canvas", 800,600);
	petAnalysis_EnergyPhot->SetTitle("Energy Phot SiPM (2 planes)");
	...
	petAnalysis_EnergyPhot->Fit("gauF2","","e",8000,10000);
	petAnalysis_EnergyPhot->Draw();

	TCanvas* c3	= new TCanvas("c3", "First canvas", 800,600);
	petAnalysis_EnergyCompt->SetTitle("Total Charge Compt");
	...
	petAnalysis_EnergyCompt->SetAxisRange(0., 15000.0,"X");
	petAnalysis_EnergyCompt->Fit("gauF2","","e",8000,10000);
